...
0xFB: 'VK_ZOOM',
        ...
        locationKeylogStructure = data.find(b'<<<KEYBOARD_BUFFER>>>')
        if locationKeylogStructure == -1:
            oOutput.Line('Keyboard buffer marker not found')
            return
        data = data[locationKeylogStructure + 32:]
        keyboardLayout, iCounterKeystrokes = struct.unpack('<II', data[0:8])
        oOutput.Line('keyboardLayout = %08x' % (keyboardLayout))
        oOutput.Line('iCounterKeystrokes = %d' % (iCounterKeystrokes))
        data = data[8:8 + iCounterKeystrokes * 20]
        ...
        while len(data) > 0:
            ...
            vkCode, scanCode, flags, time, dwExtraInfo = struct.unpack(format, data[:sizeFormat])
            data = data[sizeFormat:]
            keyTransition = flags & 0x80
            keyAlt = flags & 0x20
            oOutput.Line('%s %s %s %08x %s' % (IFF(keyTransition, 'RELEASE', 'PRESS  '), IFF(keyAlt, 'ALT', '   '), repr(chr(vkCode)), scanCode, dVirtualKeys.get(vkCode, '')))
            if dVirtualKeys.get(vkCode, '') == 'VK_LSHIFT':
                uppercase = keyTransition == 0
            elif dVirtualKeys.get(vkCode, '') == 'VK_BACK':
                if not keyTransition:
                    message = message[:-1]
            elif not keyTransition:
                character = IFF(vkCode >= 32 and vkCode < 127, chr(vkCode), '.')
                if dVirtualKeys.get(vkCode, '') == 'VK_OEM_2':
                    character = ':'
                if not uppercase:
                    character = character.lower()
                if character == '9' and message.endswith('..'):
                    message = message[:-2]
                    character = '{'
                if character == '0' and message.endswith('..'):
                    message = message[:-2]
                    character = '}'
                message += character
        oOutput.Line('Message: %s' % message)
        # ----------------------------------------------
    except:
